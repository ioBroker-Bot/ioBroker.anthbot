...
class AnthbotDevice {
    ...
    /**
     * Do not disturb value from timeSetting
     */
    get doNotDisturb() {
     ...
     *
     * @param {boolean} status Whether to enable do not disturb
     */
    async doDoNotDisturb(status) {
        ...
        if (!Array.isArray(noDisturb) || noDisturb.length != 1) {
            ...
            // Clone array with slice and overwrite active
            const args = {
                timezone: this.timeSetting?.timezone,
                timezone_secs: this.timeSetting?.timezone_secs,
                no_disturb: noDisturb.slice(),
            };
            args.no_disturb[0].active = status ? 1 : 0;
            this.log.info(`mow_regular : ${JSON.stringify(args)}`);
    ...
    setShadowState() {
        ...
        this.setStateChanged(`status.mode`, mode);
        ...
        this.setStateChanged(`${this.sn}.map.map_area`, this.shadowState.map.map_area);
    ...
    async checkMapUpdates() {
        if (
            ...
            this.shadowState.map.area_id != (await this.adapter.getStateAsync(`${this.sn}.map.area_id`))?.val ||
            this.shadowState.map.plan_id != (await this.adapter.getStateAsync(`${this.sn}.map.plan_id`))?.val
        ) {
