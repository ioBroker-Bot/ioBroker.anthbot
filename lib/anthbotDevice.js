...
class AnthbotDevice {
     ...
     */
    setShadowState() {
        // Get current mode here so offline overrride is possible below
        let mode = this.shadowState.mode.value;
        ...
        this.adapter.setStateChanged(`${prefix}.mode`, { val: mode, ack: true });
    ...
    async checkCustomAreaSchedule() {
        ...
        if (
            ...
            if (
                ...
                // that is able to be completed before dusk
                for (const customArea of customAreasLeftToday) {
                    ...
                    if (now.getTime() + customArea.estimatedElapsedTimeToUse > suncalcTimes.civilDusk.ts) {
                        this.log.warn(`Not enough time before dusk for ID ${customArea.id}`);
                    } else {
