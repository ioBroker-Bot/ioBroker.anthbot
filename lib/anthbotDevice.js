...
const CLOUD_SYNC_DELAY = 5000; // Time after submitting a command to wait before polling
...
class AnthbotDevice {
    ...
    /**
     * Does the map need (re)loading?
     *
     * @returns True if map needs loading, otherwise false
     */
    async mapNeedsLoading() {
        if (!this.mapLoaded) {
            // Not loaded at all (startup condition)
            return true;
        } else if (typeof this.shadowState.map != 'undefined') {
            // Device uses area/plan ID
            return (
        ...
        // Device must use area/map time
        // TODO: check map_time?
        return (
            this.shadowState.area_time != (await this.getStateAsync('map.area_id'))?.val ||
            this.shadowState.appointment_time != (await this.getStateAsync('map.plan_id'))?.val
        );
    }
    ...
    async checkMapUpdates() {
        if (await this.mapNeedsLoading()) {
            ...
            if (!Array.isArray(this.customAreas)) {
            ...
            if (typeof this.shadowState.map != 'undefined') {
                ...
                // Device must use area/plan(appointment) time
                // TODO: map_time?
                // TODO: correct state ID (area_time)?
                this.setStateChanged('map.area_id', areaSetting?.area_time);
                // Cannot use time from file as can a few ms different
                // Force string
                this.setStateChanged('map.plan_id', `${this.shadowState.appointment_time}`);
            }
            this.mapLoaded = true;
        }
    }
