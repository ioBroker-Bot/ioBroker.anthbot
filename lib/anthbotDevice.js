...
const TASK_START_CODES = [1017, 1018];
// ... and has finished
...
class AnthbotDevice {
    ...
    /**
     * Return true if a task appears in progress, false if available to start a new mowing task
     */
    get isWorking() {
        // Assume nothing in progress
        let isWorking = false;

        // This cannot simply be derived from mode - we need to use codeList
        if (!Array.isArray(this.codeList)) {
            ...
            if (startIndex < 0) {
                // No task start code found - just leave assumption of not working
            } else {
                ...
                if (endIndex < 0 || endIndex > startIndex) {
                    ...
                    // End must be after start - just leave assumption of not working
                }
            }
        }
        return isWorking;
     ...
     */
    planIdMatch(id1, id2) {
        return Math.abs(Number(id1) - Number(id2)) < 30;
    }
