...
const TASK_START_CODES = [1015, 1017, 1018];
// ... and has finished
    ...
    async syncDevice() {
        ...
        if (this.inPoll) {
            ...
            if (this.pollingInterval) {
                ...
                this.log.debug('pollingInterval already set in syncDevice');
                this.clearPolling();
            }
            this.pollingInterval = this.adapter.setInterval(async () => {
                if (this.inPoll) {
                    // Shouldn't happen unless polling interval is too short
                    this.log.warn('pollingInterval triggered while last poll in progress');
                } else {
                    this.log.debug('pollingInterval triggered');
                    this.pollDevice();
                }
            }, POLLING_INTERVAL_MS);
        }
