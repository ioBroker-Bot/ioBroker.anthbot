...
class AnthbotDevice {
    ...
    async createObjects() {
        ...
        const readOnlyStates = [
            ...
            // Schedule
            ['schedule.custom_areas_left_today', 'string', 'list', `List of custom areas left to start today`],
            [
                'schedule.estimated_time_left_todo_today',
                'number',
                'time.span',
    ...
    setShadowState() {
        ...
        const prefix = `${this.sn}.status`;
        this.adapter.setStateChanged(`${prefix}.active_area`, {
            val: JSON.stringify(this.shadowState.active_area.id),
            ack: true,
        });
        this.adapter.setStateChanged(`${prefix}.elec`, { val: this.shadowState.elec.value, ack: true });
        this.adapter.setStateChanged(`${prefix}.mode`, { val: this.shadowState.mode.value, ack: true });
        this.adapter.setStateChanged(`${prefix}.mowing_area`, { val: this.shadowState.mowing_area.value, ack: true });
        this.adapter.setStateChanged(`${prefix}.mowing_time`, { val: this.shadowState.mowing_time.value, ack: true });
        this.adapter.setStateChanged(`${prefix}.rtk_moved`, { val: this.shadowState.rtk.moved == 1, ack: true });
        this.adapter.setStateChanged(`${prefix}.rtk_state`, { val: this.shadowState.rtk.state == 1, ack: true });
    ...
    setCodeList() {
        const prefix = `${this.sn}.status`;
        this.adapter.setStateChanged(`${prefix}.code_list`, { val: JSON.stringify(this.codeList), ack: true });

        const lastCode = this.codeList[0];
        this.adapter.setStateChanged(`${prefix}.last_code`, { val: lastCode.code, ack: true });
        this.adapter.setStateChanged(`${prefix}.last_code_text`, { val: lastCode.event_message, ack: true });
        this.adapter.setStateChanged(`${prefix}.last_code_type`, { val: lastCode.code_type, ack: true });
    }
    ...
    async checkCustomAreaSchedule() {
        // Always figure out what is left todo today
        const customAreasLeftToday = [];
        const customAreaIdsLeftToday = [];
        ...
        for (const customArea of this.customAreas) {
            if (
                ...
                if (lastFinishPlusDaysSinceMs < today0000.getTime()) {
                    ...
                    customAreaIdsLeftToday.push(customArea.id);
        ...
        await this.adapter.setStateChanged(`${this.sn}.schedule.custom_areas_left_today`, {
            val: JSON.stringify(customAreaIdsLeftToday),
            ack: true,
        });
        await this.adapter.setStateChanged(`${this.sn}.schedule.estimated_time_left_todo_today`, {
            val: secondsLeftTodoToday,
            ack: true,
        });

        if (
            customAreasLeftToday.length > 0 &&
            ...
            if (
                ...
                if (startAreaId) {
                    this.log.info(`Scheduled start for ${startAreaId} (best pick from ${customAreasLeftToday.length})`);
