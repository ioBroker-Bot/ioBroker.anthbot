...
class AnthbotCloudApiClient {
    ...
    async checkShadowClient(serialNumber) {
        ...
        if (!this.shadowClients[serialNumber]) {
            if (this.verboseLogger) {
        ...
        if (
            !this.shadowClients[serialNumber].iotCredentials ||
            this.shadowClients[serialNumber].iotCredentials.expiresAt === null ||
            this.shadowClients[serialNumber].iotCredentials.expiresAt - Date.now() <= 60000
        ) {
            if (this.verboseLogger) {
                this.verboseLogger(`No/expired IoT Credentaials for ${serialNumber}, fetching them`);
            }
            this.shadowClients[serialNumber].iotCredentials = await this.asyncGetDeviceIotCredentials(serialNumber);
        }
    }
    ...
    async asyncGetShadowReportedState(serialNumber) {
        ...
        return await this.shadowClients[serialNumber]?.asyncGetShadowReportedState();
    }
