...
/**
 * Return a shallow copy of headers with the Authorization value masked, for safe logging
 *
 * @param {object} headers - Headers to redact
 * @returns {object} - Redacted copy of headers
 */
...
class AnthbotCloudApiClient {
    ...
    async fetch(url, options = {}) {
        ...
        if (this.verboseLogger) {
            ...
            if (options.headers) {
                this.verboseLogger(
                    `[VERBOSE] Headers: ${JSON.stringify(redactAuthorizationHeader(options.headers), null, 2)}`,
                );
            }
            if (options.body) {
    ...
    async asyncGetDeviceIotCredentials(serialNumber) {
        ...
        if (this.verboseLogger) {
            this.verboseLogger(
                `IoT credentials for ${serialNumber}: ${JSON.stringify({
                    ...iotCredentials,
